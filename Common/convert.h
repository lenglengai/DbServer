...
namespace std {
	...
	namespace convert_namespace {
		...
		template<typename __t2>
		struct Convert<void *, __t2>
		{
			__t2 operator () (void * nValue, const ConvertType_ nConvertType) const
			{
				return ( *((__t2 *)nValue) );
			}
		};
		...
		template<>
		struct Convert<void *, __i8>
		{
			__i8 operator () (void * nValue, const ConvertType_ nConvertType) const
			{
				return ( *((__i8 *)nValue) );
			}
		};
